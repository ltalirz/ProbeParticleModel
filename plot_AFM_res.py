...
for iq,Q in enumerate( Qs ):
	for ik,K in enumerate( Ks ):
		...
		if opt_dict['pos']:
			try:
				PPpos, lvec = GU.loadVecFieldNpy( dirname+'/PPpos' )
				...
				print "cannot load : " + ( dirname+'/PPpos_?.npy' ) 
		if ( ( opt_dict['df'] or opt_dict['save_df'] ) ):
			try :
				fzs, lvec  = GU.loadNpy( dirname+'/OutFz'     )
				for iA,Amp in enumerate( Amps ):
					...
					if opt_dict['save_df']:
						GU.saveNpy( dirNameAmp+'/df', dfs, lvec )
					if opt_dict['df']:
						print " plotting df : "
						PPPlot.plotImages(
				...
				print "error: ", sys.exc_info()
				print "cannot load : ", dirname+'/OutFz.npy'
