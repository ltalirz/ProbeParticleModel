...
//******************** SINGLE ORBITAL conductance:
 ...
 for (int i=0; i<NoOrb; i++){
	f += Lor(V,eig[i],eta)*sqr( Gatomsp<FUNC>(NoAt,const_orb,dr,rev_rr,radial, coesin+(i*NoAt*4) ) );
	}
 f *= Go*Norm;
 return f;
 }
 ...
 for (int i=0; i<NoOrb; i++){
	f += Lor(V,eig[i],eta)*sqr(   (len_R-al*ri.z)/len_R * Gatomsp<pzsp>(NoAt,const_orb,dr,rev_rr,radial, coesin+(i*NoAt*4) )
                                - (al*ri.x)/len_R * Gatomsp<pxsp>(NoAt,const_orb,dr,rev_rr,radial, coesin+(i*NoAt*4) )
                                - (al*ri.y)/len_R * Gatomsp<pysp>(NoAt,const_orb,dr,rev_rr,radial, coesin+(i*NoAt*4) )       );
	}
 f *= Go*Norm;
 return f;
 }
 ...
 for (int i=0; i<NoOrb; i++){
	double a = Gatomsp<pzsp>(NoAt,const_orb,dr,rev_rr,radial, coesin+(i*NoAt*4) );
	double b = Gatomsp<pxsp>(NoAt,const_orb,dr,rev_rr,radial, coesin+(i*NoAt*4) );
	double c = Gatomsp<pysp>(NoAt,const_orb,dr,rev_rr,radial, coesin+(i*NoAt*4) );
 ...
 for (int i=0; i<NoOrb; i++){
	f += Lor(V,eig[i],eta)*sqr(   (len_R-al*2*ri.z)/len_R * Gatomsp<dz2sp>(NoAt,const_orb,dr,rev_rr,radial, coesin+(i*NoAt*4) )
                                + (al*2*ri.x)/len_R * Gatomsp<dxzsp>(NoAt,const_orb,dr,rev_rr,radial, coesin+(i*NoAt*4) )
                                + (al*2*ri.y)/len_R * Gatomsp<dyzsp>(NoAt,const_orb,dr,rev_rr,radial, coesin+(i*NoAt*4) )  );
	}
 f *= Go*Norm;
 return f;
 }
 ...
 for (int i=0; i<NoOrb; i++){
	double a = Gatomsp<dz2sp>(NoAt,const_orb,dr,rev_rr,radial, coesin+(i*NoAt*4) );
	double b = Gatomsp<dxzsp>(NoAt,const_orb,dr,rev_rr,radial, coesin+(i*NoAt*4) );
	double c = Gatomsp<dyzsp>(NoAt,const_orb,dr,rev_rr,radial, coesin+(i*NoAt*4) );
 ...
 for (int i=0; i<NoOrb; i++){
	f += Lor(V,eig[i],eta)*(sqr( ( Gatomsp<FUNC>(NoAt,const_orb,drx1,rev_rrx1,radialx1, coesin+(i*NoAt*4) ) - Gatomsp<FUNC>(NoAt,const_orb,drx2,rev_rrx2,radialx2, coesin+(i*NoAt*4) ) )/Amp ) +
					 		sqr( ( Gatomsp<FUNC>(NoAt,const_orb,dry1,rev_rry1,radialy1, coesin+(i*NoAt*4) ) - Gatomsp<FUNC>(NoAt,const_orb,dry2,rev_rry2,radialy2, coesin+(i*NoAt*4) ) )/Amp )   );
	}
 f *= Go*Norm;
 return f;
 }
...
extern "C"{
 ...
 void proc_IETSspsp( int NoAt, int NoOrb, int Npoints, double V, double WF, double eta, double Amp, double* eig, double* R_, double* Rat_, double* coesin, double* tip_coes, double* cur){
 	decay = sqrt( abs(WF+WF)*eV);
	Norm = four_pi*decay;
	Vec3d * R = ( Vec3d * ) R_;
	Vec3d * Ratin = (Vec3d *) Rat_;
